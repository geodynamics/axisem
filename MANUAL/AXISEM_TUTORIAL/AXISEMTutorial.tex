...
\begin{enumerate}
    ...
    from 'surface' to 'surface with edges' (On some host systsem, the dropdown menu seems
    to be messed up, in that case go to the 'Display' context in the
    'Properties' panel on the left). You can open other vtk files to look at
    other properties of the model and the mesh. You might need to rescale the
    color range by clicking on the left-right arrow symbol in the top left.
...
\begin{enumerate}
    ...
    the \verb|prem_ani_light_100s_mzz/| \verb|Datai/xdmf_xml_0000.xdmf|
    ...
    can find a 5s period movie of these snapshots in the \verb|movie| folder on
    the desktop (use \verb|gpicview| to open the \verb|.png| files and
    \verb|???| to open the movie).
...
\begin{enumerate}
    ...
    * For more information on the folder structure of your Virtual-box, refer to
    ...
    crosscorrelation function), shift the synthetics accordingly and plot the results:

...
mkdir -p $outdir/SNAPS

echo
echo "%%%%%%%%% PROCESSING seismograms/wavefields %%%%%%%%%"
mkdir -p $outdir/SEISMOGRAMS
mkdir -p $outdir/SEISMOGRAMS/UNPROCESSED

echo ".... output in "$outdir"/OUTPUT_postprocessing ...."

../../SOLVER/UTILS/xpost_processing > OUTPUT_postprocessing
...
if ( $gnu_query == 1 ) then
    ...
    set reclist = `cat $homedir/Data/receiver_names.dat |awk '{print $1}'`
    echo "1st receiver:" $reclist[1]
    set colat = `cat $homedir/Data/receiver_names.dat |awk '{print $2}' |sed 's/00000/ /g' |awk '{print $1}'`
    set lon = `cat $homedir/Data/receiver_names.dat |awk '{print $3}' |sed 's/00000/ /g'  |awk '{print $1}'`
    set epidist = `cat $homedir/Data/receiver_pts.dat |awk '{print $1}'`
...
if ( $taup_query == 1 ) then
    ...
    set depth = `grep "source depth" simulation.info |awk '{print $1}'`
    set model = `grep Background mesh_params.h |awk '{print $5}'`

    if ( $model == 'prem' || $model == 'iasp91' ) then
	set num_rec = `wc -l Data/receiver_pts.dat |awk '{print $1}'`
	set epi_list = `tail -n $num_rec Data/receiver_pts.dat | awk '{print $1}'`

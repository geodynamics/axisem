...
set multisrc = 'false'; set newqueue = 'false'
if ( "$2" == "-s" ) then
  set srctype = `echo $3`;set multisrc = 'true'
  if ( "$4" == '-q' ) then 
    set queue = `echo $5`; set newqueue = 'true'
  endif
else if ( "$2" == '-q') then
  set queue = `echo $3`; set newqueue = 'true'
  if ( "$4" == '-s' ) then 
    set srctype = `echo $5`;set multisrc = 'true'
  endif
endif
...
if ( $multisrc == 'true' ) then
    ...
    if ( $srctype == 'moment' ) then 
      ...
      set map_mij = ( 1 2 4 6 )
      set numsim = 4; set srcapp = ( MZZ MXX_P_MYY MXZ_MYZ MXY_MXX_M_MYY )
      set srctype1 = ( "'monopole'" "'monopole'" "'dipole'" "'quadpole'")
      set srctype2 = ( "'mzz'" "'mxx_p_myy'" "'mxz'" "'mxy'" )

    else if ( $srctype == 'force' ) then 
      set numsim = 2; set srcapp = ( PZ PX )
      set srctype1 = ( "'monopole'" "'dipole'" ); set srctype2 = ( "'vertforce'" "'xforce'" )
...
echo 'source names:' $srcapp; echo 'source radiation types:' $srctype1; echo 'source components:' $srctype2

mkdir $1; cd $1; set mainrundir = $PWD
...
#	    head -n 1 $homedir/$srcfile  > $srcfile.$isrc.$isim
	    ...
	    tail -n 6 $homedir/$srcfile >> $srcfile.$isrc.$isim
    ...
    mkdir Code; cp -p $homedir/*.f90 Code; cp -p $homedir/Makefile Code
...
######## SUBMIT LOCALLY #######
     ...
     mpirun -np $nodnum ./xsem > $outputname &
  endif
...
gfortran post_processing.f90 -o xpost_processing
cd $homedir
cp -p $homedir/UTILS/xpost_processing $1
cp -p $homedir/UTILS/post_processing.f90 $1
cp -p $homedir/UTILS/post_processing.csh $1
cp -p $homedir/UTILS/plot_recfile_seis.csh $1
cp -p $homedir/UTILS/plot_recs.plot $1
cp -p $homedir/UTILS/taup_allrec.csh $1
cp -p $homedir/UTILS/plot_record_section.m $1

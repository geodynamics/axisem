...
if (file_exists) then 
   ...
   open(unit=91,file='inparam_hetero')
      ...
      allocate(het_format(num_het))
      allocate(het_file_discr(num_het))
      allocate(het_funct_type(num_het))
      allocate(rdep(num_het))
      allocate(grad(num_het))
      allocate(gradrdep1(num_het))
      allocate(gradrdep2(num_het))
	 ...
	 if ( inverseshape(ij) > 0 ) het_funct_type(ij)=het_funct_type(ij)(1:inverseshape(ij))
...
open(unit=91,file=trim(het_file_discr(hetind)))
  ...
  rhet2=-5000.;thhet2=-5000.;phhet2=-5000.;delta_vp2=-5000.;delta_vs2=-5000.;delta_rho2=-5000.
  jj=0
  ...
  do i=1,num_het
	...
	write(6,*)'reading line',j,'of',maxpts,':',rhet2(j,i),thhet2(j,i),phhet2(j,i), &
				delta_vp2(j,i),delta_vs2(j,i),delta_rho2(j,i)
     enddo
  enddo
close(91)
   ...
   do i=1,num_het
      ...
      if ( r>=rmin(i) .and. th>=thetamin(i) ) then
            ...
            if ( r<=rmax(i) .and. th<=thetamax(i) ) then
		     ...
		     call bilinear_interpolation(s,z,num_het_pts_region(i),shet(1:num_het_pts_region(i),i),&
                                     !          zhet(1:num_het_pts_region(i),i),ind,w,wsum,dr)
		     ! this finds the two points closest in z and s, doesnt always get four points!!!
		     ! weighting arbitrarily in s or z, not in 2d-distance!!!
...
dr(1)=ds1; dr(2)=ds2; dr(3)=ds3; dr(4)=ds4
ind(1)=is1;ind(2)=is2;ind(3)=is3;ind(4)=is4
...
do iel=1,nelem
   ...
   do jpol=0,npol
      do ipol=0,npol   
         ...
         vptmp = vptmp*(1. + delta_vp(1)*rand)
         vstmp = vstmp*(1. + delta_vs(1)*rand)
         lambdapost(ipol,jpol,iel) = rhopost(ipol,jpol,iel) *( vptmp*vptmp - two*vstmp*vstmp )
         mupost(ipol,jpol,iel) = rhopost(ipol,jpol,iel) * vstmp*vstmp
      enddo
   enddo
enddo
...
real(kind=8) :: vptmp,vstmp,rhotmp,s,z,r,th,gauss_val
! EDITED by S.Hempel
real(kind=8) :: r_center_gauss,th_center_gauss
real(kind=8) :: s_center_gauss,z_center_gauss,halfwidth_r,halfwidth_th
! start elastic property values
real(kind=8) :: vpst,vsst,rhost
...
! for gradient
real(kind=8) :: grad_halfwidth_r,grad_halfwidth_th
double precision :: grad_r_het,grad_th_het2,grad_th_het1
real(kind=8) :: dr_outer,dr_inner,dth_outer,dth_inner
real(kind=8) :: val,dro,dru,grad_th_val,gradwidth,grad_val
...
! Gaussian, by Tarje
   ...
   if (lpr) write(6,*)hetind,'center r,th gauss [km]:',r_center_gauss/1000.,&
      th_center_gauss/1000.
   if (lpr) write(6,*)hetind,'halfwidth r,th gauss [km]:',halfwidth_r/1000.,halfwidth_th/1000.
...
! ADDED by S.Hempel
elseif (het_funct_type(hetind)=='sinus' .or. het_funct_type(hetind)=='trian' .or. het_funct_type(hetind)=='gss1d' &
	!.or. het_funct_type(hetind)=='spher' 
	.or. het_funct_type(hetind)=='inclp' .or. het_funct_type(hetind)=='inclr' &
        .or. het_funct_type(hetind)=='const' ) then 
   decay=1.d0
...
! determine velocity ontop of heterogeneity and calculate future elastic properties of the heterogeneity
      if (.not. rdep(hetind) ) then
		 ...
		 if (lpr) write(6,*)'depth-independent variation!'
	         if (lpr) write(6,*)hetind,'elastic properties set to (vp,vs,rho):',r_het2,vpst,vsst,rhost
	 else
		idom = minloc(abs(discont-r_het1(hetind)),1)
		   ...
		   !write(6,*)'discmin?',ij,abs(discont(ij)-r_het1(hetind))
	         ...
	         !rhost = rhost* (1.+delta_rho(hetind))
		 if (lpr) write(6,*)'depth-independent variation!'
	         if (lpr) write(6,*)hetind,'elastic properties set to (vp,vs,rho):',r_het1,vpst,vsst,rhost
	 endif
      endif
...
! if gradient
      ...
      if (lpr) write(6,*)hetind,'center r [km],th [deg]:',het_funct_type(hetind),r_center_gauss/1000.,&
           th_center_gauss*180./pi
      if (lpr) write(6,*)hetind,'halfwidth r [km],th [deg]:',het_funct_type(hetind),halfwidth_r/1000.,&
		th_het1(hetind)*180./pi,th_het2(hetind)*180./pi,halfwidth_th*180./pi
      if ( grad(hetind) .and. lpr ) write(6,*)hetind,'gradhalfwidth r [km],th [deg]:',&
		het_funct_type(hetind),grad_halfwidth_r/1000.,grad_halfwidth_th*180./pi
...
! find elements within heterogeneity
   ...
   do iel=1,nelem
      ...
      rmax=0.999*max(r1,r2,r3,r4);  thetamax=0.999*max(th1,th2,th3,th4)
		...
		( grad(hetind)  .and. rmax<=grad_r_het .and. thetamax<=grad_th_het2 .and. thetamin>=grad_th_het1)) then 
      ...
      do jpol=0,npol
         do ipol=0,npol
	       ...
	       gradwidth = 0.
	       if ( grad(hetind) ) gradwidth = (gradrdep1(hetind)-gradrdep2(hetind))*1000./r_het1(hetind) / &

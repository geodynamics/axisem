...
call nc_open(dirname,ncid_in,nc_disp_varid)
!************************************************************
...
! >>>>> load seismograms <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   if (.not. interpolate_seis) then ! choosing closest location
      ...
      call nc_read_seis(ncid_in, nc_disp_varid, ind_rec, nt, seis_snglcomp)
           ...
           open(unit=20,file=trim(recname(irec))//'_'//trim(interp_method)//'_raw'//appidur//'_sim1.ascii')
           open(unit=21,file=trim(recname(irec))//'_'//trim(interp_method)//'_raw'//appidur//'_sim2.ascii')
           open(unit=22,file=trim(recname(irec))//'_'//trim(interp_method)//'_raw'//appidur//'_sim3.ascii')
           open(unit=23,file=trim(recname(irec))//'_'//trim(interp_method)//'_raw'//appidur//'_sim4.ascii')

           do it=1,nt
              write(20,*) it, seis_snglcomp(it,1,1)
              write(21,*) it, seis_snglcomp(it,1,2)
              write(22,*) it, seis_snglcomp(it,1,3)
              write(23,*) it, seis_snglcomp(it,1,4)
           enddo

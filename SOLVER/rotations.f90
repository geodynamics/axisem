...
!-----------------------------------------------------------------------------
subroutine save_google_earth_kml(srccolat1,srclon1,rcvcolat,rcvlon,num_rec_glob,fname,receiver_name)

use data_proc, only : appmynum

integer, intent(in)                    :: num_rec_glob
real, intent(in) :: srccolat1,srclon1,rcvcolat(1:num_rec_glob),rcvlon(1:num_rec_glob)
character(len=40), intent(in)     :: receiver_name(num_rec_glob)
...
! components
if (rot_rec=='sph') then
   comp(1) = 'r'; comp(2) = 'ph'; comp(3) = 'th'
elseif (rot_rec=='cyl') then
   comp(1) = 's'; comp(2) = 'ph'; comp(3) = 'z'
elseif (rot_rec=='enz') then
   comp(1) = 'E'; comp(2) = 'N'; comp(3) = 'Z'
!!!!!!!! NOT SURE IF THAT IS THE RIGHT ORDER !!!
else
   write(6,*)'unknown receiver component system',rot_rec
stop
endif
 
!open(unit=88,file=infopath(1:lfinfo)//'/src_rec_'//fname//appmynum//'.kml')

...
! take surface/crustal values as constraint on ns resolution
      ...
      if (dump_mesh_info_screen) write(6,*)'ns_ref initial estimate from crust   :',ns_ref_surf
 
! take ICB value as constraint on ns resolution 
      if (solid_domain(ndisc)) then 
         ns_ref_icb1 = estimate_ns(pts_wavelngth,rdisc_top(ndisc), &
                           vs(ndisc,1),period)*2**nc_init
      else ! bottom layer is a fluid
         ns_ref_icb1 = estimate_ns(pts_wavelngth,rdisc_top(ndisc), &
                                 vp(ndisc,1),period)*2**nc_init
      endif

      if (dump_mesh_info_screen) write(6,*)'ns_ref initial estimate from icb     :',ns_ref_icb1

      if (resolve_inner_shear) then 
!         ns_ref = ns_ref_icb1
! still need to resolve the crust...
...
!     write(6,*)'NEWTEST:',ns_ref,2**nc_init*nproc_target
      if ( mod(ns_ref, 2**nc_init*nproc_target*2) /= 0 ) &
      ns_ref = (2**nc_init*nproc_target*2)* &
               ( ns_ref/(2**nc_init*nproc_target*2) + 1 )
...
!      maxh_icb = vs(ndisc,1)*period/pts_wavelngth/max_spacing(npol)
!      rmin = maxh_icb*dble(ns_ref/2**nc_init/2+1)
...
!      write(6,*)'RMIN NEXT MAXH:',real(floor(rmin/maxh_icb))*maxh_icb
      ...
      ndisc_stop = ndisc
      if (ndisc_stop == ndisc) rdisc_bot(ndisc_stop) = rmin 
...
!========================
      do idom =1, ndisc_stop
!========================
!         write(6,*) 
!        write(6,*) ' IDOM, r_disc_top ', idom,rdisc_top(idom)/1000.
         current_radius = rdisc_top(idom)
         memorydz = .false.
         do while (current_radius > rdisc_bot(idom) ) 
            call compute_dz_nz(idom,rdisc_bot,&
                 current_radius,dz,ds,current,memorydz, &
...
! scale for smallest grid spacing in GLL clustering
      ...
      dt = courant * min(minval(ds_glob/vp_arr), &
                     minval(dz_glob/vp_arr))/dble(npol) * minh/aveh

      if (dump_mesh_info_screen) write(6,*)'TIME STEP:',dt

      if (dump_mesh_info_files) then
       open(unit=665,file=diagpath(1:lfdiag)//'/inner_shear.txt')
       write(665,*)resolve_inner_shear
...
!!$      write(20,*) ns_glob 
!!$      write(20,*) nz_glob
!!$      write(20,*) rmin/6371.e3
!!$      write(20,*) 1. 
!!$      do iz_glob = 1, nz_glob
!!$         write(20,*) 2.*dz_glob(nz_glob-iz_glob+1)/(6371.e3-rmin)
!!$      end do
!!$      close(20) 
...
!  deallocate(iclev_glob)
!  deallocate(dz_glob)

end subroutine create_subregions

!dk compute_dz_nz----------------------------------------------------------
subroutine compute_dz_nz(idom,rdisc_bot,&
                         current_radius,dz,ds,current,memorydz, &
                         icount_glob,ic,ns_ref)
...
!    layers between the actual layer and the bottom of the subregion?)
      ...
      if (nz_trial >= 3) then
!       write(6,*) ' WE SHALL COARSEN AND SQUEEZE '
       ns_trial = ns_ref
       ns_ref = ns_ref / 2
       ic = ic + 1
       memorydz = .true.
...
!    write(6,10) current_radius,current_radius-dz,dz, real(ns_trial), & 
!                real(ns_ref),dz/ds
    current_radius = current_radius -dz

!10 format(40(1pe12.5,2x))
...
deallocate(eta,xi_k,dxi,spacing_eta,spacing_xi)

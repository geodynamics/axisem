...
!--------------------------------------------------------------------------
!====================================
! Quantities needed in the solver
!====================================
!
! global numbering:
!nel_glo, el_glob(nel_glo)=ipt_glo
!nel_sol, el_sol(nel_sol)=ipt_glo
!nel_flu, el_sol(nel_flu)=ipt_glo
!
! axial masking:
!nel_sol_ax, el_sol_ax(iel)=iel_sol
!nel_flu_ax, el_sol_ax(iel)=iel_flu 
!
! surface elements:
!nel_surf, el_surf(iel)=iel_sol

! SOLID-FLUID BOUNDARIES, TOP=CMB, BOT=ICB:
!nel_solflu_top (CMB)
!nel_flusol_bot (ICB)
! 
!el_solflu_top(nel_solflu_top)=iel_sol (CMB)
!el_flusol_top(nel_solflu_top)=iel_flu (CMB)
!
!el_solflu_bot(nel_flusol_bot)=iel_sol (ICB)
!el_flusol_bot(nel_flusol_bot)=iel_flu (ICB)
! 
!bdry_coords_top(ipol,iel,1)=s
!bdry_coords_top(ipol,iel,2)=z
! 
!bdry_coords_bot(ipol,iel,1)=s
!bdry_coords_bot(ipol,iel,2)=z
! 
!! TYPES
!el_typ(iel)=
!'solide' ! solid
!'soliax' ! solid axis
!'solflu' ! solid-fluid boundary (solid side)
!'soflax' ! solid-fluid boundary, axis (solid side)
!'socoar' ! solid, coarsening
!'socoax' ! solid, coarsening, axis
!'surfac' ! solid, free surface
!'surfax' ! solid, free surface, axis
!'sollin' ! central region including buffer
!'solmpi' ! solid, at processor boundary
!
!'fluide' ! fluid
!'fluiax' ! fluid, axis
!'flusol' ! fluid-solid boundary (fluid side)
!'flsoax' ! fluid-solid boundary, axis (fluid side)
!'flcoar' ! fluid, coarsening
!'flcoax' ! fluid, coarsening, axis

! INSTEAD: type definitions:
!
! rheology: sol,flu 
! rheology%axis y,n
! rheology%mpi :y,n
! rheology%boundary: y,n
! rheology%coars: y,n
! rheology%spher: y,n (no=central region and buffer)
! 
!
!dk define_regions---------------------------------------------------------
...
! define center of mass for each element
  allocate(scom(neltot)) ; scom(:) = 0.d0 
  allocate(zcom(neltot)) ; zcom(:) = 0.d0 
  allocate(thetacom(neltot)) ; thetacom(:) = 0.d0
  allocate(rcom(neltot)) ; rcom(:) = 0.d0 
  allocate(rmin_el(neltot)) ; rmin_el(:) = 0.d0 
  allocate(rmax_el(neltot)) ; rmax_el(:) = 0.d0 
!af  
...
!  TNM SOBER and dirty
!!$   if (iel <= neltot /2 ) then      ! NH
!!$    scom(iel) = sgll(1,npol/2,iel)
!!$    zcom(iel) = zgll(1,npol/2,iel)
!!$   else    
!!$    if (southern) then
!!$     scom(iel) = sgll(npol-1,npol/2,iel)
!!$     zcom(iel) = zgll(npol-1,npol/2,iel)
!!$    else
!!$     scom(iel) = sgll(1,npol/2,iel)
!!$     zcom(iel) = zgll(1,npol/2,iel)
!!$    end if
...
!TARJE SEPT 2006: Not sure why this hasn't been done beforehand...
  scom(iel) = sgll(npol/2,npol/2,iel)
  zcom(iel) = zgll(npol/2,npol/2,iel)
...
!->>>>>>>>>>>>>>>>>>>>>>><
  ...
  enddo !j
  close(67661); close(67662); close(67666)
  close(67671); close(67672); close(67673); close(67674); close(67675)
  close(7775); close(7776); close(7777); close(7778)
...
! reference coordinates 
  ...
  do ibelem = 1,neltot
     ...
     if ( abs(rcom(ibelem)-bdry_radius(1))< abs( zgll(0,0,belem(1,1) )-zgll(npol,npol,belem(1,1) ) ) ) then
...
! TNM 2011: Lower bound iglob_fluid negative.... ISSUE?
!                 write(67664,*)ibelem,iglob((ibelem-1)*(npol+1)**2+ npol*(npol+1) + ipol + 1 )
!                 write(67668,*)sgll(ipol,npol,ibelem),zgll(ipol,npol,ibelem), &
!                    iglob_fluid( (inv_ielem_fluid(ibelem)-1)*(npol+1)**2+ npol*(npol+1) + ipol + 1 )
              endif
              ...
              if (rcom(ibelem)>bdry_radius(1)) then ! above, solid
                 ...
                 write(67665,*)sgll(ipol,npol,ibelem),zgll(ipol,npol,ibelem), &
                      iglob_solid( (inv_ielem_solid(ibelem)-1)*(npol+1)**2+ npol*(npol+1) + ipol + 1 )
                 
              else ! below, fluid
                 write(67664,*)ibelem,iglob( (ibelem-1)*(npol+1)**2 + ipol + 1 )
! TNM 2011: Hmmmm
 !                write(67668,*)sgll(ipol,0,ibelem),zgll(ipol,0,ibelem), &
!                      iglob_fluid((inv_ielem_fluid(ibelem)-1)*(npol+1)**2 + ipol + 1 )
              endif
     ...
     if (abs(rcom(ibelem)-bdry_radius(2))< abs( zgll(0,0,belem(1,2) )-&
           ...
           do ipol=0,npol
              ...
              if (rcom(ibelem)>bdry_radius(2)) then ! above, fluid
                 write(67664,*)ibelem,iglob( (ibelem-1)*(npol+1)**2 + ipol + 1)
! TNM 2011: Some issue with lower bound of iglob_fluid.... we render this unimportant at this point.
!                 write(67668,*)sgll(ipol,0,ibelem),zgll(ipol,0,ibelem), &
!                      iglob_fluid( (inv_ielem_fluid(ibelem)-1)*&
!                      (npol+1)**2 + ipol + 1 )
              else ! below, solid
              ...
              if (rcom(ibelem)>bdry_radius(2)) then ! above, fluid
                 write(67664,*)ibelem,iglob((ibelem-1)*(npol+1)**2+ &
                               npol*(npol+1) + ipol + 1 )
! TNM 2011: Some issue with lower bound of iglob_fluid.... we render this unimportant at this point.
! Maybe not unimportant. But probably caused by some ill-posed indexing in this loop, like neltot...
 !                write(67668,*)sgll(ipol,npol,ibelem),zgll(ipol,npol,ibelem), &
  !                   iglob_fluid( (inv_ielem_fluid(ibelem)-1)* &
!                      (npol+1)**2+ npol*(npol+1) + ipol + 1 )
     ...
     close(67663);close(67664);close(67665);close(67667);close(67668)
!->>>>>>>>>>>>>>>>>>>>>>><
  end if
!->>>>>>>>>>>>>>>>>>>>>>><
...
!--------------------------------------------------------------------------
!
!dk define_my_boundary_neighbour-------------------------------------------
...
! i.e. counting above AND below. 
  ...
  double precision :: mytheta, hertheta ! TNM PC...crap + AF Geek
...
! tolerance = 10.*min_distance_nondim
...
!    write(6,*)'BOUNDARY NEIGHBOR SEARCH:',myel,mytheta; call flush(6)
...
!    write(6,121) ibelem, jbelem,mytheta, hertheta, abs(hertheta-mytheta)*rbound, tolerance
!   if ( jbelem==nbelem(j) .and. .not. foundone) then 
...
!    write(6,*)'                  FOUND :',my_neighbour(ibelem,j), &
!                                 thetacom(belem(my_neighbour(ibelem,j),j))
    call flush(6)
   end do
  end do

! 121 format(i3,2x,i3,2x,5(1pe12.5,2x))
!  12 format(i3,i4,i4,4(1pe15.5))
...
!  if ( (rmin_el(belem(ibelem,j))+rmax_el(belem(ibelem,j))) > 2.*rbound ) then
...
!    write(6,11)'AB:',j,ibelem,abovecount, &
!               rcom(belem(ibelem,j))*router/1000.,rbound*router/1000.
! 11 format(a4,i3,i4,i4,2(1pe12.2))
...
! very crude way to accomodate the case of having the buffer layer just below the ICB.
! ...i.e., the jpol indices are switched for 45 < theta < 135 deg
        if (eltypeg(belem(my_neighbour(ibelem,j),j))/='curved') then 
           if (dump_mesh_info_screen) write(6,*)'ELTYPE:',j,abovecount,eltypeg(belem(my_neighbour(ibelem,j),j))
...
!    write(6768,15)j,ibelem,abovecount,my_neighbour(ibelem,j),belem(ibelem,j),&
!                 belem(my_neighbour(ibelem,j),j),&
!                 bdry_solid_el(abovecount,j),bdry_fluid_el(abovecount,j)
!    15 format(i3,2(i4),i8,4(i8))
...
! I am below (only for output check)
  ...
  if (dump_mesh_info_files) &  
  write(131313,30)rup,rdown,thetaup,thetadown
...
!~~~~~~~~~~~~~~~~~~~~~~~
  close(131313); close(6565); close(6464); close(6465)

  write(6,*)'Done with boundary neighbor search.'; call flush(6) 

  if (dump_mesh_info_files) then
!!$   open(unit=6363,file=diagpath(1:lfdiag)//'/bdry_coords_elems_boundcoords.dat')
!!$   open(unit=6767,file=diagpath(1:lfdiag)//'/fort.6767')
!!$   open(unit=8787,file=diagpath(1:lfdiag)//'/fort.8787')
!!$   open(unit=8788,file=diagpath(1:lfdiag)//'/fort.8788')
  end if
!!$do j=1,nbcnd
!!$!   write(6,*)'Boundary #:',j; call flush(6)
!!$   do ibelem=1,nbelem(j)/2
!!$!      write(6,*)'bdry elem #:',ibelem,nbelem(j)/2; call flush(6)
!!$      if (dump_mesh_info_files) &
!!$           write(6767,11)'BY:',j,ibelem,bdry_jpol_solid(ibelem,j),&
!!$           bdry_solid_el(ibelem,j),&
!!$           sqrt(bdry_s(npol/2,ibelem,j)**2+&
!!$           bdry_z(npol/2,ibelem,j)**2)*router/1000., &
!!$           sqrt(sgll_solid(npol/2,bdry_jpol_solid(ibelem,j),&
!!$           bdry_solid_el(ibelem,j))**2 + &
!!$           zgll_solid(npol/2,bdry_jpol_solid(ibelem,j),&
!!$           bdry_solid_el(ibelem,j))**2)*router/1000., &
!!$           sqrt(sgll_fluid(npol/2,bdry_jpol_fluid(ibelem,j),&
!!$           bdry_fluid_el(ibelem,j))**2 + &
!!$           zgll_fluid(npol/2,bdry_jpol_fluid(ibelem,j),&
!!$           bdry_fluid_el(ibelem,j))**2)*router/1000.
!!$11    format(a4,i3,2(i4),i6,3(1pe12.2))
!!$      if (dump_mesh_info_files) & 
!!$           write(6363,20)bdry_s(npol/2,ibelem,j)*router/1000., &
!!$           bdry_z(npol/2,ibelem,j)*router/1000.,&
!!$           ibelem,my_neighbour(ibelem,j)
!!$      
!!$   enddo
!!$enddo
!!$j=2
!!$do ibelem=1,nbelem(j)/2
!!$if (dump_mesh_info_files) &
!!$write(8787,19)ibelem,bdry_solid_el(ibelem,j),eltypeg(belem(ibelem,j)),&
!!$             thetacom(belem(ibelem,j))*180./pi,&
!!$             sqrt(sgll_solid(npol/2,0,bdry_solid_el(ibelem,j))**2+&
!!$                  zgll_solid(npol/2,0,bdry_solid_el(ibelem,j))**2)*router/1000., &
!!$             sqrt(sgll_solid(npol/2,npol,bdry_solid_el(ibelem,j))**2+&
!!$                  zgll_solid(npol/2,npol,bdry_solid_el(ibelem,j))**2)*router/1000.
!!$
!!$
!!$if (dump_mesh_info_files) &
!!$write(8788,*)sgll_solid(npol/2,0,bdry_solid_el(ibelem,j))*router/1000.,&
!!$             zgll_solid(npol/2,0,bdry_solid_el(ibelem,j))*router/1000., &
!!$             bdry_solid_el(ibelem,j),eltypeg(belem(ibelem,j))
!!$enddo
!19 format(i3,i6,a8,3(1pe12.3))
!20 format(2(1pe12.3),2(i5))
...
!  write(6,10) j, nbelem(j)
!10 format(15x, '# of boundary elements for interface ', i2, ' : ', i4)
...
!dk check_boundary-----------------------------------------------------------
  ...
  itest = 0
!  dtest = sqrt(abs(1.-(s/rbound)**2-(z/rbound)**2))  
!  if (dtest < dmax) itest = 1
